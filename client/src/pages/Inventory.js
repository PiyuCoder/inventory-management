  ...
  return (
    <>
      {!inventories.length ? (
        ...
        <div className=" w-full h-full bg-white flex flex-col py-10 px-4">
          {/* <p className=" ms-10"> {`Welcome, ${user.name}`}</p> */}
          <h2 className=" text-start w-full font-bold text-lg md:text-[30px]">
            Inventory Tracker
          </h2>
          <p className=" text-start w-full text-xs mb-8">
            Track your Inventories here
          </p>
          <div className=" self-end">
            <button
              className=" bg-green-500 text-white p-3  rounded-lg font-bold me-10 text-xs md:text-sm"
            ...
            <table className=" w-full">
              <thead className=" bg-cyan-950 text-white ">
                <tr className=" border-2 gap-10 ">
                  ...
                  <th
                    className={` text-sm border-l-white border-r-white border-2`}
                  >
                    Vendor
                  </th>
                  <th className=" text-sm border-l-white border-r-white border-2">
              ...
              <tbody>
                ...
                {filteredInventories?.map((inventory, index) => (
                  <tr
                    className=" border-2 hover:bg-slate-100 text-xs md:text-sm"
                    key={inventory._id}
                  >
          ...
          {isAddInventory && (
            <AddInventory setIsAddInventory={setIsAddInventory} />
          )}
        </div>
      )}
    </>
